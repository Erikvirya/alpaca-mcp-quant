...
def find_strike_for_delta(S, T, r, sigma, target_delta=0.20):
    ...
    return round(mid, 2) # ETFs have tighter strike intervals than raw BTC

def run_ibkr_simulation():
    print("Downloading BTC-USD for IBKR IBIT Simulation (2021-2026)...")
    # Using 1h data to filter for US market hours (better proxy for ETF trading)
    df = yf.download('BTC-USD', start='2021-01-01', end='2026-03-13', interval='1h', progress=False)
    if isinstance(df.columns, pd.MultiIndex): df.columns = df.columns.get_level_values(0)
    
    # Filter for US Market Hours (approx 14:00 to 21:00 UTC depending on DST)
    df.index = df.index.tz_convert('America/New_York')
    df = df.between_time('09:30', '16:00')
    
    # Resample to Daily to represent ETF daily bars
    df_daily = df.resample('D').agg({'Open': 'first', 'High': 'max', 'Low': 'min', 'Close': 'last'}).dropna()
    
    df_daily['returns'] = np.log(df_daily['Close'] / df_daily['Close'].shift(1))
    df_daily['RV_20'] = df_daily['returns'].rolling(20).std() * np.sqrt(252) # 252 for ETF
    ...
    initial_cash = 100000
    leverage = 1.5
    margin_interest_annual = 0.065 # 6.5% IBKR Pro Rate
    opt_commission = 0.65
    slippage_pct = 0.0125 # 1.25% slippage (Realistic for IBIT)
    ...
    print(f"\n--- IBKR IBIT SIMULATION (1.5x LEVERAGE) ---")
