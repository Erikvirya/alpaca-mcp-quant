...
async def execute_vectorbt_strategy(
    ...
    try:
        ...
        if pd is None or np is None or vbt is None:
            missing = []
            if pd is None:
            ...
            return json.dumps(
                {
                    "error": {
                        "message": "Missing required dependency(ies)",
                        "missing": missing,
                    }
                },
                separators=(",", ":"),
        ...
        if pf is None:
            return json.dumps(
                {
                    "error": {
                        "message": "strategy_code did not define `pf`. Define a VectorBT portfolio as `pf = vbt.Portfolio...`",
                    }
                },
                separators=(",", ":"),
            )
